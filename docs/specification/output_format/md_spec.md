...
- 行间公式前后禁止有额外的空行，双$$符号顶格，前后不能有空格，并且独占一行
...
除了以上几个`符号`, 如果上述符号出现在文本段落（行内公式除外），则都需被转义（图片，列表、代码、行间公式、表格中的文字无需转义）。

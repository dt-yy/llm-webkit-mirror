...
class TestPoliticalDetector:
    ...
    def test_init(self, mock_load_model, mock_auto_tokenizer):
        ...
        mock_load_model.assert_called_once_with(
            os.path.join("custom_model_path", "model.bin")
        )
...
def test_decide_political_by_str():
    with patch(
        "llm_web_kit.model.policical.get_singleton_political_detect"
    ) as mock_get_singleton_political_detect, patch(
        "llm_web_kit.model.policical.decide_political_func"
    ) as mock_decide_political_func:
...
def test_update_political_by_str():
    with patch(
        "llm_web_kit.model.policical.decide_political_by_str"
    ) as mock_decide_political_by_str:

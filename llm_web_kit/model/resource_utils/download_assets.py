...
def decide_cache_dir():
    """
    Get the cache directory for the web kit.
    The
...
def download_auto_file(
    resource_path: str, target_path: str, md5_sum: str = "", exist_ok=True
) -> str:
    """
    Download a file from a given resource path (either an S3 path or an HTTP URL) to a target path on the local file system.
    ...
    if os.path.exists(target_path):
        ...
        if md5_sum:
            ...
            if file_md5 == md5_sum:
                logger.info(
                    f"File {target_path} already exists and has the correct md5 sum"
                )
                return target_path
            else:
                logger.info(
                    f"File {target_path} already exists but has incorrect md5 sum."
                )
    ...
    logger.info(
        f"Downloading {resource_path} to {target_path}, but first to a temporary file"
    )
    ...
    with tempfile.NamedTemporaryFile() as tmp_file:
        ...
        try:
            ...
            if local_asset_size != total_size_in_bytes:
                raise Exception(
                    f"Downloaded asset size {local_asset_size} does not match expected size {total_size_in_bytes}"
                )

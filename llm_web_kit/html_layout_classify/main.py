...
def __read_sample_of_layout_id(to_process_file_path):
    ...
    with open(to_process_file_path, 'r') as f:
        for line in f:
            ...
            if data['layout_id'] == cur_layout_id:
...
def __do_page_classify(samples:list) -> tuple[str, float]:
    ...
    if len(samples) < 3:
        logger.error(f"samples of layout_id {samples[0]['layout_id']} is less than 3")
    # 进行2次分类
    ...
    if classify_res_top_2[0]['pred_label'] == classify_res_top_2[1]['pred_label']:  # 如果1和2的分类结果一致，则直接返回1的分类结果
        ...
        classify_3 = MODEL.predict(html_str_inputs[3])
...
def __process_one_layout_file(result_save_dir, to_process_file_path):
    ...
    result_file_path = result_save_dir + Path(to_process_file_path).name
    ...
    for samples_of_layout_id in __read_sample_of_layout_id(to_process_file_path):
        ...
        file_buffer.write(json.dumps(classify_res, ensure_ascii=False) + '\n')
...
def __report_status(server_addr, file_path, status):
    """更新server上的状态."""
    UPDATE_STATUS_URL = f'{server_addr}/update_status'
    requests.post(UPDATE_STATUS_URL, json={'file_path': file_path, 'status': status})
